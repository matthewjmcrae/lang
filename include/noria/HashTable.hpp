...
namespace noria {
  ...
  public:
    ...
    template <bool IsConst> class BasicIterator {
      ...
      using table_type = std::conditional_t<IsConst, const HashTable, HashTable>;
      table_type* table_ = nullptr;
      size_type index_ = 0;

      BasicIterator(table_type* table, size_type index) : table_(table), index_(index) {
        skipVacant();
      }
    ...
    public:
      ...
      friend bool operator==(const BasicIterator& left, const BasicIterator<OtherConst>& right) {
    ...
    explicit HashTable(size_type bucketCount = kMinimumCapacity, const Hasher& hash = Hasher{},
    ...
    HashTable(size_type bucketCount, Val tombstone, const Hasher& hash = Hasher{},
              const KeyEqual& equal = KeyEqual{})
        : missingValue_(std::move(tombstone)), hasher_(hash), equal_(equal) {
      initialize(bucketCount);
    }

    HashTable(std::initializer_list<value_type> values, size_type bucketCount = kMinimumCapacity,
              const Hasher& hash = Hasher{}, const KeyEqual& equal = KeyEqual{})
        : hasher_(hash), equal_(equal) {
      initialize(bucketCount);
      reserve(values.size());
    ...
    HashTable(InputIt first, InputIt last, size_type bucketCount = kMinimumCapacity,
              const Hasher& hash = Hasher{}, const KeyEqual& equal = KeyEqual{})
        : hasher_(hash), equal_(equal) {
      initialize(bucketCount);
      insert(first, last);
    ...
    [[nodiscard]] size_type max_size() const noexcept { return slots_.max_size(); }
    ...
    template <typename M> std::pair<iterator, bool> insertOrAssign(Key key, M&& value) {
    ...
    Val& operator[](Key&& key) { return try_emplace(std::move(key)).first->second; }
    ...
    [[nodiscard]] bool contains(const Key& key) const { return findIndex(key) != npos; }

    [[nodiscard]] size_type count(const Key& key) const { return contains(key) ? 1 : 0; }
    ...
    [[nodiscard]] size_type bucket_count() const noexcept { return slots_.size(); }
    ...
    [[nodiscard]] float max_load_factor() const noexcept { return maxLoadFactor_; }

    void max_load_factor(float value) {
      if (!(value > 0.0F && value < 1.0F)) {
        throw std::invalid_argument("HashTable max_load_factor must be between 0 and 1");
      }
    ...
    void rehash(size_type requestedBuckets) {
      requestedBuckets = nextPrime(maximum(requestedBuckets, capacityFor(size_)));
      ...
      for (Slot& slot : oldSlots) {
        if (slot.state == State::Occupied) {
          tryEmplaceWithoutGrowth(slot.value->first, std::move(slot.value->second));
        }
      }
    }
  ...
  private:
    ...
    std::pair<iterator, bool> tryEmplaceImpl(K&& key, Args&&... args) {
      ...
      occupy(probe.index, std::forward<K>(key), std::forward<Args>(args)...);
    ...
    template <typename K, typename M> void tryEmplaceWithoutGrowth(K&& key, M&& mapped) {
      const Probe probe = locateForInsert(key);
      occupy(probe.index, std::forward<K>(key), std::forward<M>(mapped));
    }

    template <typename K, typename... Args> void occupy(size_type index, K&& key, Args&&... args) {
      Slot& slot = slots_[index];
      const bool reused = slot.state == State::Deleted;
      slot.value.emplace(std::piecewise_construct, std::forward_as_tuple(std::forward<K>(key)),
    ...
    [[nodiscard]] size_type findIndex(const Key& key) const {
      ...
      for (size_type examined = 0; examined < slots_.size(); ++examined) {
        ...
        if (slot.state == State::Occupied && equal_(slot.value->first, key)) {
          return index;
        }
    ...
    [[nodiscard]] std::pair<size_type, size_type> hashes(const Key& key) const {
      const size_type hash = static_cast<size_type>(hasher_(key));
      const size_type buckets = slots_.size();
      return {hash % buckets, 1 + mix(hash ^ static_cast<size_type>(0x9e3779b9U)) % (buckets - 1)};
    }

    static size_type advance(size_type index, size_type step, size_type capacity) noexcept {
      index += step;
    ...
    [[nodiscard]] size_type maxEntries(size_type buckets) const noexcept {
      return static_cast<size_type>(static_cast<double>(buckets) * maxLoadFactor_);
    }
    ...
    [[nodiscard]] size_type capacityFor(size_type entries) const {
      ...
      return static_cast<size_type>(std::ceil(static_cast<double>(entries) / maxLoadFactor_));
    }
    ...
    static constexpr size_type maximum(size_type left, size_type right) noexcept {
